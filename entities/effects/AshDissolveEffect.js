...
export default class AshDissolveEffect {
    ...
    spawn(target, depth = 30, options = {}) {
        ...
        const baseSpiritWidth = spirit.width;
        const baseSpiritHeight = spirit.height;
        spirit.setSize(baseSpiritWidth * config.spiritScale, baseSpiritHeight * config.spiritScale);

        const cloneBaseWidth = clone.displayWidth;
        const cloneBaseHeight = clone.displayHeight;
        this.scene.tweens.add({
            targets: clone,
            y: target.y - config.riseDistance,
            x: target.x + Phaser.Math.FloatBetween(-config.driftX, config.driftX),
            alpha: 0,
            displayWidth: cloneBaseWidth * 0.92,
            displayHeight: cloneBaseHeight * 0.84,
        ...
        this.scene.tweens.add({
            ...
            alpha: 0,
            width: baseSpiritWidth * config.spiritScale * 0.78,
            height: baseSpiritHeight * config.spiritScale * 1.24,
