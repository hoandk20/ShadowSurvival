...
function getEnemyMaxHealth(waveNumber, enemyType) {
    ...
    const clampedWave = Phaser.Math.Clamp(Math.round(waveNumber), 1, INSIDE_CHURCH_MAX_WAVE);
    const progress = (clampedWave - 1) / (INSIDE_CHURCH_MAX_WAVE - 1);
...
function regularWave(enemies = []) {
    return {
        durationSeconds: INSIDE_CHURCH_REGULAR_WAVE_DURATION,
...
function bossWave(enemies = []) {
    return {
        durationSeconds: INSIDE_CHURCH_BOSS_WAVE_DURATION,
...
function createWavePlans() {
    // Roster mapping vs church_sanctuary:
    // slime -> dino
    // bugmonster -> moth_woman
    // lamia -> firer
    // miniboss ailen -> medusa
    // miniboss skeleton -> minotau
    // final boss plant -> black_widow
    const medusaBoss = miniBoss('medusa', 2, { maxHealth: 4000, damage: 50, armor: 7, scale: 1.7 });
    const minotauBoss = miniBoss('minotau', 3, { maxHealth: 10000, damage: 50, armor: 10, scale: 1.7 });
    const medusaPair = miniBoss('medusa', 2, { maxHealth: 8000, damage: 50, armor: 7, scale: 1.7 });
    const minotauTrio = miniBoss('minotau', 3, { maxHealth: 10000, damage: 50, armor: 10, scale: 1.7 });
    const minotauFive = miniBoss('minotau', 5, { maxHealth: 10000, damage: 15 });

    const waves = [];
    for (let wave = 1; wave <= INSIDE_CHURCH_MAX_WAVE; wave += 1) {
        let enemies = [];
        let bossEnemies = [];

        if (wave <= 5) {
            enemies = [enemy('dino', 12)];
            if (wave === 5) bossEnemies = [medusaBoss];
        } else if (wave <= 10) {
            const phaseOffset = wave - 6;
            enemies = [
                enemy('dino', buildPhaseWeight(11, phaseOffset, -1, 8)),
                enemy('widow', buildPhaseWeight(8, phaseOffset, 1, 8)),
                enemy('succubus', 2),
                enemy('firer', 2)
            ];
            if (wave === 10) bossEnemies = [minotauBoss];
        } else if (wave <= 15) {
            const phaseOffset = wave - 11;
            enemies = [
                enemy('dino', buildPhaseWeight(8, phaseOffset, -1, 6)),
                enemy('widow', buildPhaseWeight(10, phaseOffset, -1, 8)),
                enemy('succubus', 2),
                enemy('firer', 2),
                enemy('bomber', buildPhaseWeight(1, phaseOffset, 1, 1))
            ];
            if (wave === 15) bossEnemies = [medusaPair, minotauTrio];
        } else {
            const phaseOffset = wave - 16;
            enemies = [
                enemy('moth_woman', buildPhaseWeight(13, phaseOffset, 1, 13)),
                enemy('dino', buildPhaseWeight(6, phaseOffset, -1, 4)),
                enemy('widow', buildPhaseWeight(8, phaseOffset, -1, 7)),
                enemy('succubus', 2),
                enemy('firer', 2),
                enemy('bomber', 2)
            ];
            if (wave === 20) bossEnemies = [minotauFive, finalBoss('black_widow')];
        }
...
export function getInsideChurchSpawnRate(waveNumber = 1) {
    const clampedWave = Phaser.Math.Clamp(Math.round(waveNumber), 1, 7);
    const progress = (clampedWave - 1) / 6;
    return 0.5 + (1.5 * progress);
}

export function createInsideChurchScenario(sharedEnemyHealthMilestones = []) {
    return {
        normalSpawnPerSecond: 0.5,
        waveSpawnRateByWave: getInsideChurchSpawnRate,
